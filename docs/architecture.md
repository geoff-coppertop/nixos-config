...
| `pkgs/` | Custom package builds: `search-light`, `connect-iq-sdk-manager-cli` (`framework-control` moved upstream to nixpkgs). `pywiim` + `home-assistant-wiim` (a community Wiim/`linkplay` workaround) lived here and were removed when that workaround was reverted — see [docs/smart-home.md § Wiim](smart-home.md#wiim-reverted-to-core-linkplay-pending-live-verification) |
