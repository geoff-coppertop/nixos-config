...
| `custom.autoRip.scratchMaxAgeDays` | A daily timer deletes anything under `stateDir`'s raw/transcode scratch past this age. ARM's own `DELRAWFILES` only cleans up on a *successful* job; this is the safety net for a failed or aborted one, which otherwise lingers forever |
| `custom.autoRip.hardwareEncode` | Swaps in a QSV-enabled HandBrakeCLI (`pkgs/handbrake-qsv.nix`, a from-source build nixpkgs doesn't ship) and passes `/dev/dri` plus a declared `render` group GID in. Enabled on `excelsior` — untested end-to-end, confirm a real rip uses the GPU |
| `custom.mediaManager` | tinyMediaManager, naming and scraping the library Jellyfin serves — headless (its CLI, not its GUI), triggered by `custom.mediaSort`'s `OnSuccess=`, not a standing web UI |
| `custom.mediaSort` | Timer-driven (`modules/media-sort.nix`) — sorts ARM's local `completed/` output into `movies/`/`tv/` by querying ARM's own `job.video_type` (it already identifies movie vs series), not by guessing from filenames. `importDir` adds a second, DB-less leg for `custom.mediaRipping`'s `import-disc` |
| `custom.mediaRipping` | Manual/fallback optical toolkit (`modules/media-ripping.nix`) — MakeMKV, HandBrake, whipper, and `import-disc` for data discs (e.g. DivX/Xvid rips) that need copying rather than decrypting/transcoding. `import-disc` has no metadata source of its own, so `--type`/`--show`/`--season` at invocation is what `custom.mediaSort`'s `importDir` leg later classifies by |
