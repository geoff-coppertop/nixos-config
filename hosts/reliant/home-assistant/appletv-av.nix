...
# else it could be displaying once the Apple TV sleeps, and no case (e.g.
# switching to a different input) where powering off immediately would be
# wrong. If that ever changes — another source gets added to the receiver —
# revisit this, since switching away from the Apple TV to use that other
# source would then trigger an unwanted shutoff.
#
