...
#
# A bare path literal such as `./files/face.png`, in a .nix file that's
# itself part of this flake, resolves to a *subpath* of the single
# whole-repo store copy Nix makes of `self` before evaluation. Coercing it
# to a string — `.source = ./x`, `${./x}` inside a builder, `toString ./x`
# — embeds a value whose store identity moves whenever any tracked file
