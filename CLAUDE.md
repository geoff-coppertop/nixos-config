...
- **When asking the user to run a command themselves, give each one its own fenced code block** — one command (or a short pipeline) per block, no inline narration mixed in — so it can be copied with a single click rather than hand-edited first.
