...
- **When watching a PR, rely on the activity subscription alone — don't also schedule a self check-in (`send_later`/routines) as a backstop.** The subscription's webhook delivery is enough; don't add scheduled-check machinery on top of it.
- **Every change ships with test steps unless it's a docs-only change.** Put concrete, specific test steps in the PR description — exact commands and the exact expected output/result, not "verify it works." State plainly which steps you actually ran versus which are for the user to run (e.g. anything needing hardware or a deployed host you can't reach). The sole exception is a docs-only change (Markdown/comments with no runtime surface); everything else — code, Nix, automations, config — needs them.
- **Remote `nixos-rebuild` deploys to a headless host use `--target-host thomasga@<host>.local` (bare hostname isn't resolvable) and `--sudo` (not `--use-remote-sudo`, which is deprecated).** Every headless host follows defiant's pattern: `security.sudo.wheelNeedsPassword = false;` since the SSH key check is the real access gate, so the deploy needs no interactive password.
