...
- **Standing rules apply every time the situation recurs, not just the first time in a session.** Confirming one force-push/history-rewrite doesn't carry over to the next one with different mechanics — re-summarize and wait again. The commit-squashing rule applies to your own commits as you write them, not only when critiquing someone else's history — if a later commit in the same not-yet-merged branch just revises what an earlier one added (bug fix or scope change, doesn't matter which), squash them before calling the history "clean," rather than needing a second correction round.
