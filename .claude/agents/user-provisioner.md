...
- **The machine's capabilities, as opposed to the person's preferences** →
  `machine-provisioner`. `profiles/desktop/` (which desktop environment runs,
  app pruning, pipewire, logind idle/suspend) and `profiles/dev/` (Podman, the
  Connect IQ toolchain, USB debug probes) are system-layer files: they say what
  the machine can do, not what one person likes. Your half is the
  personalization on top — theme, wallpaper, dconf, which optional GUI apps a
  user opts into.
...
- `programs.git`/`fzf`/`starship`/`zoxide` must not write the paths the
  `dotfiles` input already owns — home-manager hard-errors at eval time on
  collision. The links live with the user who has them, not in
  `users/common/`: `users/thomasga/shell.nix` places `config.fish` and sets
  `enableFishIntegration = false` for fzf/starship/zoxide (that file's
  `config.fish` runs their inits itself); `users/thomasga/git.nix` places
  `git/config` and `git/commit-template`. `users/common/cli/{fzf,starship,
  zoxide}.nix` only enable their tool and leave integration at the upstream
  default — correct for a user who does *not* take dotfiles from there. Check
  the consuming user's file, not a shared one, before touching any of this.
