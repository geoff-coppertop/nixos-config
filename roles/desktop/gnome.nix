{pkgs, ...}: let
  battery-sleep = pkgs.writeShellScript "battery-suspend-then-hibernate" ''
...
in {
  ...
  # GNOME Shell on Wayland does not reliably call SetIdleHint on the logind
  # session, so logind's IdleAction=suspend-then-hibernate rarely fires.
  # swayidle uses ext-idle-notify-v1 (GNOME Shell 44+) to track actual
  # compositor idle time and is the primary idle-hibernate trigger on battery.
  # Timeout matches idle-delay (240 s) + IdleActionSec (60 s) = 300 s.
  systemd.user.services."swayidle-hibernate" = {
    description = "Auto-hibernate on battery after idle";
    ...
    serviceConfig = {
      ...
      Restart = "always";
      RestartSec = "10s";
      ExecStart = pkgs.writeShellScript "swayidle-hibernate-runner" ''
        exec ${pkgs.swayidle}/bin/swayidle -w \
          timeout 300 '${battery-sleep}'
      '';
    };
  };
  # Hold the logind idle inhibitor while on AC power as a secondary guard.
  # On battery the service exits immediately (no inhibitor held).
