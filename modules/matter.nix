{
  ...
  # root certificates, before the websocket server binds. DCL has been
  # observed serving a certificate that fails strict ASN.1 parsing in the
  # `cryptography` library, raising an uncaught ValueError inside
  # `paa_certificates.py` — `server.start()` never completes, the websocket
  # port never opens, and systemd still shows "active (running)" since the
  # process doesn't crash. See
  # https://github.com/NixOS/nixpkgs/issues/377136 (open as of 2026-08-07;
  # this flake's pinned nixpkgs revision carries no workaround).
  #
  ...
  # project-chip/connectedhomeip at runtime) baked into the package, and
  # patch `fetch_certificates()` to install those instead of hitting
  # DCL/GitHub. Trade-off: no automatic pickup of newly issued vendor PAA
  # certs — see docs/smart-home.md § Matter for how to re-pin
  # `pinnedPaaCertsRev` below.
  ...
  # Standalone regression test for the patch below: calls the patched
  # fetch_certificates() directly and asserts it installs the pinned certs,
  # with no network and no dependency on server.py/zeroconf. Needed because
  # the *only* upstream test exercising fetch_certificates() at all is
  # test_server_start, which is deselected below for an unrelated reason
  # (see disabledTests) — without this file, the patched function would
  ...
  # call site in server.py: it's the real top-level, importable function, so
  # this is a smaller/more robust substituteInPlace match than a call-site
  # snippet, and lets paaCertificatesPinnedTest above exercise it directly.
  matterServerPackage = pkgs.python-matter-server.overrideAttrs (oldAttrs: {
    # Overriding postPatch changes this derivation's hash, so it's built
    # (and test-suite-checked) locally/in CI instead of pulled prebuilt —
    # which surfaced an unrelated failure: test_server_start calls
    # server.start(), which goes on to device_controller.start(), where
    # zeroconf's IPV6_MULTICAST_IF socket setup raises "OSError: [Errno 92]
    # Protocol not available" — no real IPv6 multicast in this build
    # sandbox (aarch64). Passes on nixpkgs's own Hydra, so this is one
    # observed failure on this build host, not a general sandbox fact.
    # Deselect only this test — doCheck stays on, and
    # paaCertificatesPinnedTest above covers the patched function directly.
...
in {
  ...
  config = mkIf cfg.enable {
    # python-matter-server: the Matter controller/fabric Home Assistant's
    # "matter" integration connects to over WebSocket at
    # ws://localhost:5580/ws. Adding the integration and commissioning each
    # device (the Aqara M2 hub, bridging the U100 locks and FP1e presence
    # sensor) are one-time UI steps; this just installs the backend. Runs on
    # the same host as HA (`reliant`), so HA always reaches it over localhost.
