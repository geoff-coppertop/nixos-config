{
  ...
  # part of the regular Nix-rendered `settings` attrset: client_secret has to
  # be substituted at runtime (Authelia's own Go-template "secret" function
  # reading an absolute path directly, the way the jwks doc example does it)
  # rather than baked into the world-readable Nix store, and Authelia's
  # config-file merging replaces whole list values
  # (identity_providers.oidc.clients is a list) rather than merging list
  # items, so the entire client entry has to live in one file.
  # `{{ secret ... | mindent N "|" | msquote }}` has no manual surrounding
  # quotes — msquote already produces a correctly-quoted YAML scalar, so
  # adding our own would double-quote it. Field values confirmed against
  ...
  options.custom.authelia = {
    ...
    ldap = {
      bindDn = mkOption {
        ...
        description = ''
          ...
          users is ou=people under the base DN. This user must exist in
          lldap (custom.lldap.bootstrap.users) and belong to the
          lldap_strict_readonly built-in group, never lldap_admin — Authelia
          only ever reads user/group attributes, and scoping the bind
          account to read-only avoids handing a web-facing service full
          lldap admin rights.
        '';
      };
    ...
    protectedSubdomains = mkOption {
      ...
      description = ''
        ...
        isn't protected until its own router adds
        middlewares = ["authelia@file"]). Never include
        custom.lldap.subdomain or custom.authelia.subdomain themselves —
        see § Self-Lockout Rule.
      '';
    };
    ...
    oidc = {
      enable = mkEnableOption ''
        ...
        from the LDAP-backed forward-auth above, for services that already
        have their own real login (forward-auth would just add a redundant
        second login). See docs/homelab-network.md § OIDC Provider.
      '';
      ...
      issuerPrivateKeyFile = mkOption {
        ...
        description = ''
          ...
          signing key — an RSA private key, PKCS#8 or PKCS#1, at least 2048
          bits. Maps to
          services.authelia.instances.main.secrets.oidcIssuerPrivateKeyFile,
          which nixpkgs' authelia module auto-templates into
          identity_providers.oidc.jwks at startup — jwks is never set
          directly in settings here. Generate with:
      ...
      hmacSecretFile = mkOption {
        ...
        description = ''
          ...
          (identity_providers.oidc.hmac_secret), signing OIDC JWTs. Maps to
      ...
      homeAssistant = {
        enable = mkEnableOption ''
          ...
          component. Requires custom.authelia.oidc.enable.
        '';
        ...
        redirectUri = mkOption {
          ...
          description = ''
            ...
            callback path (/auth/oidc/callback) at whatever subdomain
            modules/home-assistant.nix's Traefik route hardcodes ("home").
            Not consumed by Home Assistant's own config (hass-oidc-auth
            derives its callback URL from HA's own base URL) — this is what
            Authelia's client registration expects the browser back on.
          '';
        };
        ...
        clientSecretHashFile = mkOption {
          ...
          description = ''
            ...
            NOT the raw secret. Authelia's
            identity_providers.oidc.clients[].client_secret field stores
            only the digest it validates an incoming secret against, and
            there's no oidcClientSecretFile in nixpkgs' secrets.* fields, so
            this is read directly at runtime via Authelia's "secret"
            function in a generated settingsFile below — same as
            custom.authelia.ldap.bindPasswordFile.

            Generate both the raw secret and its digest together:
              nix run nixpkgs#authelia -- crypto hash generate pbkdf2 --variant sha512 --random
            The plaintext output goes into Home Assistant's own
            auth_oidc.client_secret (not managed by this repo); the digest
            goes into this file, and only this file.
          '';
        };
      };
    };
  ...
  config = mkIf cfg.enable (mkMerge [
    {
      ...
      # Confirmed on a real switch: authelia-main only depended on
      # lldap.service being up, not on lldap-bootstrap.service having
      # finished reconciling the "authelia" bind account it logs in with —
      # this raced and crash-looped twice before Restart=on-failure got it
      # up on the third try. Blocking on lldap-bootstrap.service explicitly
      # (a oneshot unit that only reports done once bootstrap.sh exits)
      # removes the race instead of relying on the restart policy.
      ...
      services.authelia.instances.main = {
        ...
        # fields (jwt/oidc/session/storage only), so it goes through
        # Authelia's own env-var secret convention instead. This bypasses
        # systemd's LoadCredential, so the file itself must be readable by
        # this instance's own system user (authelia-main), which the agenix
        # owner needs to be set to.
        ...
        settings = {
          ...
          # Authelia refuses to start if a secret is set both ways.
          authentication_backend.ldap = {
            ...
            # (ou=people/ou=groups search bases, uid/cn attributes).
          };

          storage.local.path = "/var/lib/authelia-main/db.sqlite3";

          # No SMTP configured yet — password-reset/notification emails
          # write to a local file instead of sending. A real gap in the
          # flow, tracked as a TODO. See hosts/reliant/README.md § Authelia.
    ...
    # OIDC provider — a separate mkIf/mkMerge entry from the block above
    # since it only applies when custom.authelia.oidc.enable is set,
    # independent of the base cfg.enable gate.
    (mkIf cfg.oidc.enable {
      services.authelia.instances.main = {
        ...
        # Home Assistant's client entry lives entirely in its own generated
        # settingsFile — see homeAssistantOidcClientFile's comment above.
        # No identity_providers.oidc.jwks here: nixpkgs' authelia module
        # auto-generates that from secrets.oidcIssuerPrivateKeyFile above.
        settingsFiles = mkIf cfg.oidc.homeAssistant.enable [homeAssistantOidcClientFile];
      };
    })

    # The forward-auth middleware. A router opts in with
    # middlewares = ["authelia@file"] — see lib/traefik-route.nix's
    # middlewares parameter. Split from the route registration below since
    # both write services.traefik.dynamicConfigOptions.http and assigning
    # both from one attrset literal would be a Nix "attribute already
    # defined" error; the module system merges the two fragments instead,
    # the same way dns.nix/home-assistant.nix/zigbee.nix coexist on it.
    ...
    # Authelia's own login portal route — deliberately no authelia
    # middleware here either, see the assertion above and
