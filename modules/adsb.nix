  ...
  options.custom.adsb = {
    ...
    locationEnvFile = mkOption {
      ...
      description = ''
        ...
        location/coordinates.age file, also consumed by
        custom.home-assistant.locationEnvFile for HA's own core location
        config — this module only reads the LAT/LON fields, not the
        LOCATION_ELEVATION field that secret also carries.
      '';
    };
  };
