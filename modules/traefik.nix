  ...
  options.custom.traefik = {
    ...
    acme = {
      ...
      environmentFile = mkOption {
        type = types.str;
        description = "Path to env file containing DNS provider credentials (agenix-managed). Format: VAR=value — for the cloudflare provider used on reliant, lego reads CLOUDFLARE_DNS_API_TOKEN=xxx, not CF_DNS_API_TOKEN.";
      };
  ...
  config = mkIf cfg.enable {
    security.acme = {
      ...
      certs.${domain} = {
        ...
        group = "traefik";
        # lego's zone-discovery (walking SOA records up from
        ...
        # own local-authority resolver to find where the zone lives broke
        # that walk entirely: "cloudflare: failed to find zone ca.: zone
        # could not be found" — it walked straight past coppertop.ca to the
        # public TLD, and every renewal since failed the same way, leaving
        # Traefik serving the pre-apex wildcard-only cert indefinitely.
        # Pointing lego at a public resolver avoids asking the zone about
        # itself.
        dnsResolver = "1.1.1.1:53";
        # Without this, Traefik never learns a new cert exists — it keeps
        # serving whatever it loaded at startup (the self-signed fallback,
        # if ACME hadn't yet succeeded) until manually restarted. Cert
        # issuance succeeded but curl still saw the self-signed chain until
