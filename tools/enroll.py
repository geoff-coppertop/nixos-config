...
def enroll_machine(machine: str, username: str = "thomasga") -> bool:
    ...
    if age_public_key:
        ...
        if age_choice == "1":
            ...
            if identity_file.exists():
                ...
                print()
                print(f"  Back up {identity_file} to Bitwarden before proceeding.")
                print("  This is the only copy of this machine's decryption key until")
                print("  it's installed — if it's lost before being backed up, the only")
                print("  recovery path left is the offline admin key.")
    ...
    if age_public_key:
        result = run(["nix", "run", ".#secret-rekey"], cwd=str(root), check=False)
        print()
